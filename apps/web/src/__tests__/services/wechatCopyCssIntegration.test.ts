...
describe("wechat copy css integration", () => {
  it("resolves inline var() values with scope-aware computed values", () => {
    ...
    const css = `
      #ahafair {
        --ahafair-font-size: 14px;
        --ahafair-text-color: #123456;
        --ahafair-paragraph-margin: 18px;
      }
      #ahafair p {
    ...
    expect(output).not.toContain("var(--ahafair-font-size)");
    expect(output).not.toContain("var(--ahafair-text-color)");
    expect(output).not.toContain("var(--ahafair-paragraph-margin)");
  });
  ...
  it("keeps literal var() text inside quoted string values", () => {
    ...
    const css = `
      #ahafair p {
        ...
        color: var(--ahafair-text-color, #222222);
      }
    `;
  ...
  it("materializes visual theme styles without remaining css variables", () => {
    ...
    expect(output).not.toContain("var(--ahafair-");
    expect(output).not.toMatch(/--ahafair-[\w-]+\s*:/);
  });
  ...
  it("propagates #ahafair background-color to child blocks after normalization (#52)", () => {
  ...
  it("materializes inherited text color to avoid ui theme leakage", () => {
    const container = document.createElement("div");
    container.innerHTML = `
      <section id="ahafair" style="color: var(--text-primary);">
        <div class="callout">
