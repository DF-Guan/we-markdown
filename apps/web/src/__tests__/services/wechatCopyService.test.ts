...
describe("wechatCopyService clipboard strategy", () => {
  beforeEach(() => {
    ...
    mocked.processHtmlMock.mockReturnValue(
      '<section id="ahafair"><p style="margin-top:18px;margin-bottom:18px;">段落A</p><p style="margin-top:18px;margin-bottom:18px;">段落B</p></section>',
    );
  ...
  it("prefers native execCommand copy", async () => {
    ...
    await copyToWechat("test", "#ahafair p { margin: 18px 0; }");
  ...
  it("uses rendered plain text instead of raw markdown in Clipboard fallback", async () => {
    ...
    await copyToWechat("# 标题", "#ahafair p { margin: 18px 0; }");
  ...
  it("converts mac-sign svg to img before clipboard write", async () => {
    mocked.processHtmlMock.mockReturnValue(
      '<section id="ahafair"><pre class="custom"><span class="mac-sign" style="padding: 10px 14px 0;"><svg xmlns="http://www.w3.org/2000/svg" width="45" height="13" viewBox="0 0 450 130"></svg></span><code class="hljs">const a = 1;</code></pre></section>',
    );
    ...
    await copyToWechat(
      "test",
      "#ahafair pre.custom > .mac-sign { display: block; }",
      { showMacBar: true },
    );
